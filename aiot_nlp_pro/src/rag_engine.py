...
def init_rag_db(db_path: str = 'rag_engine.db'):
    """初始化RAG引擎数据库，创建必要的表"""
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS source_knowledge (
            ...
            knowledge TEXT NOT NULL  -- 存储JSON格式的字符串数组
        )
    ''')
    
    # 创建索引以提高查询速度
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_source_id ON source_knowledge(source_id)')
...
def populate_source_knowledge(sample_data: dict = None):
    """
    向数据库填充素材-知识点关联数据
    
    Args:
        sample_data: 样本数据字典 {source_id: [knowledge1, knowledge2, ...]}
    """
    if sample_data is None:
        sample_data = {
            "pdf_123": ["勾股定理", "直角三角形", "历史背景"],
            "ppt_456": ["函数图像", "变换规律", "应用案例"],
            "video_789": ["函数图像", "动态演示", "互动演示"]
        }
    
    conn = sqlite3.connect('rag_engine.db')
    cursor = conn.cursor()
    
    # 批量插入数据
    for source_id, knowledge_list in sample_data.items():
        knowledge_json = json.dumps(knowledge_list)
        cursor.execute(
            "INSERT OR REPLACE INTO source_knowledge (source_id, knowledge) VALUES (?, ?)",
            (source_id, knowledge_json)
        )
    
    conn.commit()
    conn.close()
    
    print(f"✅ 已填充 {len(sample_data)} 条素材-知识点关联数据")
...
class HybridRagEngine:
    ...
    def get_relevant_documents(self, query: str, source_id: Optional[str] = None) -> List[Document]:
        """获取与查询相关的文档，可指定来源ID（素材ID）"""
        # 先进行标准检索
        ...
        if source_id:
            # 从数据库中获取该素材关联的知识点
            related_knowledge = self._get_related_knowledge(source_id)
            # 从检索结果中筛选与关联知识点相关的文档
            filtered_docs = [doc for doc in docs if any(k in doc.page_content for k in related_knowledge)]
            if filtered_docs:
                return filtered_docs
        
        return docs[:5]  # 返回前5个文档

    def _get_related_knowledge(self, source_id: str) -> List[str]:
        """从数据库获取素材关联的知识点（实际部署实现）"""
        ...
        try:
            # 查询关联的知识点
            cursor.execute(
                "SELECT knowledge FROM source_knowledge WHERE source_id = ?",
                (source_id,)
             )
            ...
            if result:
                ...
                try:
                    ...
                    # 如果JSON格式错误，返回空列表
                    print(f"⚠️ JSON解析错误: {result[0]} for source_id={source_id}")
                    return []
            else:
                # 没有找到关联知识
                print(f"⚠️ 未找到素材ID {source_id} 的关联知识点")
                return []
